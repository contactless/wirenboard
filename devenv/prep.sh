...
# add deb-src entries in sources.list
/bin/echo "deb-src $REPO $RELEASE main" >>$ROOTFS_DIR/etc/apt/sources.list
/bin/echo "deb-src $REPO $RELEASE-updates main" >>$ROOTFS_DIR/etc/apt/sources.list
...
chr apt-get install -y devscripts python-virtualenv equivs build-essential \
    ...
    cdbs libpng12-dev libqt4-dev linux-headers-4.1.15-imxv5-x0.1 quilt
